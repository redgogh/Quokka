...
void RenderDevice::SetBuffer(Pipeline pipeline, const std::string &name, size_t offset, size_t range, Buffer buffer)
{
...
void RenderDevice::SetTexture(Pipeline pipeline, const std::string& name, Texture texture, VkSampler sampler)
{
    if (!pipeline->descriptorSetInfos.contains(name))
        throw std::runtime_error(qk_format("[vulkan] error cannot found descriptor info by name '%s'", name.c_str()));
